...
const serverLoggers = new Map<string, Apperio>();
 ...
 * ```
 */
export function createServerLogger(config: ServerLoggerConfig): Apperio {
 ...
 */
export async function destroyServerLogger(config: {
  apiKey: string;
  projectId: string;
}): Promise<void> {
  const cacheKey = `${config.projectId}:${config.apiKey}`;
 ...
 */
export async function destroyAllServerLoggers(): Promise<void> {
